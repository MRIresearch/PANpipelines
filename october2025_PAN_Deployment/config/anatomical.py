...
def efc(img, framemask=None, decimals=4):
    ...
    #CPU - inhomogeneity avoid nans
    matdata = (img[framemask == 0] / b_max) * np.log((img[framemask == 0] + 1e-16) / b_max)
    matdata2 = np.ma.sum(np.ma.array(matdata,mask=np.isnan(matdata) | np.isinf(matdata)))
